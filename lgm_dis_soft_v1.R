...
lgm_proxy_data<- read.csv('proxy_o2_d14c_so2.csv') #13ka diff SO
...
par(mar=c(3.5,3.5,1,1)); par(ps = 10, cex = 1, cex.main = 1); par(mgp=c(2.25,0.75,0)); par(las=1);par(tck=-0.01)
plot(-999,-999, xlim=c(10,-200), ylim=c(10,-200), xlab=expression(paste(Delta*Delta^14*C," (\u2030)")), ylab=expression(paste(Delta*O[2]," (",mu*mol/kg,")")))
grid (NULL,NULL, lty = 6, col = adjustcolor("cornsilk2", alpha=0.7)) 
...
text(-30,-145, 'soft', col='cadetblue4', cex=0.9)
text(-150,-80, 'diseq', col='coral3', cex=0.9)
...
points(-101, -168-DO2_sol, pch=1, col=adjustcolor('grey37', alpha=0.8), cex=1.6) #SO O2 anom from clim 

legend('bottomright', pch=c(24,25,22,21), col=c('grey17','grey17','grey17','grey37'),pt.bg=adjustcolor(c('grey57','grey57','grey57','grey87'), alpha=0.8), c('Pac mid','Pac deep','Atl deep','SO deep'), cex=0.9, bty='n')
...
####substitutions 
########
#age component of lgm c14 in years
lgm_ddic_soft_mean*a/(-10/83)
#285.5587 yrs
#dis component of lgm c14 in years
